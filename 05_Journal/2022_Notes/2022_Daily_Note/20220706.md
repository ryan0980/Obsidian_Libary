...
int mount = 0; // indicator, if mounted, it will turn 1, else it will be 0;
// initial be 0

int check = -1; // see if operation has been accepcted
uint32_t op;    // operation

uint32_t make_op(int DiskID, int BlockID, int Command, int Reserved) // make operation code
{
  return (DiskID << 28) + (BlockID << 20) + (Command << 14) + (Reserved);
}

int mdadm_mount(void)
{
  if (mount == 0) // in unmount status, able to be mount
  {
    // op = 0000000000000000000000000000000;
    // printf("\ncheck:  %d\n", check);
    op = make_op(0, 2, 0, 4);         // operation code will be all 0
    check = jbod_operation(op, NULL); // return 0 on success and -1 on fail
    // printf("\ncheck:  %d\n", check);
    if (check == 0)
    {

      mount = 1; // change mount status indicator to mounted(1)
      return 1;  // return successful signal
    }
  }
  return -1; // return failed signal
}
...
int mdadm_read(uint32_t addr, uint32_t len, uint8_t *buf) // read operation
{
  ...
  op = make_op(Disk_Address, Block_Address, JBOD_SEEK_TO_DISK, 0);
  check = jbod_operation(op, NULL);
  op = make_op(Disk_Address, Block_Address, JBOD_SEEK_TO_BLOCK, 0);
  check = jbod_operation(op, NULL);
  // ready to read
  void *out = malloc(JBOD_BLOCK_SIZE);
  op = make_op(Disk_Address, Block_Address, JBOD_READ_BLOCK, 0); //
...
int mdadm_write(uint32_t addr, uint32_t len, const uint8_t *buf)
{
  ...
  op = make_op(Disk_Address, Block_Address, JBOD_WRITE_BLOCK, 0);
  check = jbod_operation(op, out);
  if (Current_Buf_Position_Read == 256)
  {
    Current_Buf_Position_Read = 0;
